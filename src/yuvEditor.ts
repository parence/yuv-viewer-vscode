...
class PawDrawDocument extends Disposable implements vscode.CustomDocument {
	...
	static async create(
		...
		// const fileData = await PawDrawDocument.readFile(uri);
		// const fileData = (await readYUV(uri.fsPath, [720, 1280])).y;
		// const reader = new Reader(uri.fsPath, {width: 1280, height: 720}, 8, '420');
		return new PawDrawDocument(uri);//, delegate);
	}
...
export class PawDrawEditorProvider implements vscode.CustomReadonlyEditorProvider<PawDrawDocument> {
	...
	private static readonly viewType = 'vyu.view';
